...
;(function(cunij){
  ...
  cunij.UnoSlider = function(options, base){
    var cunij = jQuery;
    var $this = base;
    ...
    var defaults_cfg = cunij.extend(true, defaults, $.UnoSlider.defaults);
    var preset_base_cfg = cunij.extend({}, defaults, $.UnoSlider.defaults);
    var cfg = cunij.extend(true, defaults_cfg, options);
    ...
    $this.main = {
      ...
      scaleSlides: function(){
        ...
        var div = cunij('<div class="unoslider_slider_area"></div>').css({overflow: 'hidden', width: '100%', height: $this.height(), position: 'absolute'});
        ...
        slides.find('.unoslider_slider_area div img').each(function(){
          ...
          if(typeof cunij(this).attr('height') === 'undefined')
            cunij(this).height(theImage.height);

          if(typeof cunij(this).attr('width') === 'undefined')
            cunij(this).width(theImage.width);
        });
      ...
      generateCurtain: function(content){
        ...
        var blocks_area = cunij('<div class="unoslider_blocks_area"></div>').css({overflow: 'hidden', width: $this.width(), height: $this.height(), position: 'absolute', top: '0px', left: '0px', 'z-index': 3}).appendTo($this[0]);
        var blocks = cunij('<div class="unoslider_blocks"></div>').css({width: $this.width(), height: $this.height(), position: 'relative'}).appendTo(blocks_area);

        // generate
        while (generateBlocks) {
          content_cube = cunij('<div class="unoslider_cube_content"></div>').css({
            position: 'absolute',
            left: -X,
            top: -Y,
            width: $this.width(),
          ...
          cunij('<div id="block_' + i + '" class="unoslider_cube"></div>').appendTo(blocks).css({
            'z-index': 2,
            position: 'absolute',
            display: 'none',
            top: Y,
      ...
      presets: function(){
        ...
        if(typeof cfg === 'object' && typeof cfg.preset === 'object'){
          ...
          if(typeof cfg.preset[index] == 'string'){
            ...
            cfg.animation = cunij.extend({}, preset.animation, cfg.preset[index].animation);
            cfg.block = cunij.extend({}, preset.block, cfg.preset[index].block);
          }
    ...
    $this.layer = {
      ...
      show: function(slide){
        ...
        var layers = cunij(slide).find('.unoslider_layers').children().length;
        var delays = [];
        var delay = 250;
        var duration = 300;
        var i;
        ...
        i = 1;
        cunij(slide).find('.unoslider_layers').children().each(function(){
          delays.push(delay * i);
          i++;
        });
    ...
    $this.timebar = {
      ...
      // create timebar
      set: function(){
        return cunij('<div class="unoslider_timer"></div>').css({'z-index': 1}).width('0%').appendTo($this[0]);
      },
      ...
      stop: function(){
        var timebar = $this.find('.unoslider_timer');
        cunij(timebar).stop().width('0%').hide();
      },
    ...
    $this.navigation = {
      ...
      generate: function(){
        ...
        if(cfg.navigation){
          var container = cunij('<div class="unoslider_navigation_container"></div>').insertAfter($this[0]);

          if(cfg.slideshow){
            var play = cunij('<a title="'+cfg.navigation.play+'" class="unoslider_play unoslider_navigation">'+cfg.navigation.play+'</a>').appendTo(container);
            var stop = cunij('<a title="'+cfg.navigation.stop+'" class="unoslider_pause unoslider_navigation">'+cfg.navigation.stop+'</a>').appendTo(container);
          ...
          cunij('<a title="'+cfg.navigation.prev+'" class="unoslider_left unoslider_navigation">'+cfg.navigation.prev+'</a>').appendTo(container);
          cunij('<a title="'+cfg.navigation.next+'" class="unoslider_right unoslider_navigation">'+cfg.navigation.next+'</a>').appendTo(container);
          ...
          cunij(".unoslider_navigation").css({
            'z-index': 5,
            position: 'absolute'
          });
        }
      ...
      indicator: function(){
        ...
        controls = cunij('<div class="unoslider_indicator"></div>').css({'z-index': 6});

        !cfg.indicator.position ? controls.appendTo(appendTo) : controls.insertAfter(appendTo);

        slides.each(function(slide) {
          cunij('<a title="' + (slide + 1) + '" class="'+activeClass+'">' + (slide + 1) + '</a>').appendTo(controls).bind("click", function(){
    ...
    $this.slideshow = {
      ...
      start: function(){
        cunij('.unoslider_preloader').remove();
      ...
      changeTo: function(changeTo, handChange){
        ...
        cunij.each(layerAnimation, function(i){
    ...
    $this.tooltip = {
      ...
      generate: function(){
        if(cfg.tooltip){
          ...
          slides.each(function(){
            img = cunij(this).find('img');

            if(img.length >= 1){
              caption = img.attr('title');
              if(typeof caption !== 'undefined'){
                img.removeAttr('title');
                cunij('<div class="unoslider_caption_data">'+caption+'</div>').appendTo(this).hide();
              }
            }
          });

          cunij('<div class="unoslider_caption"></div>').hide().css({'z-index': 5}).appendTo($this);
        }else{
      ...
      show: function(slide){
        var html = cunij(slide).find('.unoslider_caption_data').html();
    ...
    $this.preloader = {
      ...
      init: function(callback){
        ...
        slides.children().each(function(){
          var bg = cunij(this).css("background-image");
          if(bg !== 'none'){
        ...
        if(images.length > 0){
          cunij('<div class="unoslider_preloader"></div>').css({
            position: 'absolute',
            textAlign: 'center',
            width: '100%'
          }).prependTo($this);
          ...
          images.each(function(){
            var img = new Image;
            var $thisImg = cunij(this);

            img.src = this.src;

            if (!img.complete) {
              cunij(img).bind("load error", function(){
          ...
          cunij(lists[0]).css("display", "block").animate({
            opacity: 1
          }, 500);

          if(cunij.isFunction(callback)){
            callback.call();
          }
        }
      },
      ...
      show: function(img, callback, imagesCount){
        ...
        switch(cfg.preloader){
          case 'spinner':
            cunij(loader).empty();
            cunij('<div class="unoslider_spinner"></div>').css({width: '100%', height: '100%'}).appendTo(loader);
            break;
          default:
            cunij(loader).empty();
            var progress = cunij('<span></span>').css({width: (loaded / imagesCount * 100).toFixed(0)+'%'});
            cunij(loader).css({'padding-top': $this.height() / 2 +'px', height: '50%'});
            cunij('<div class="unoslider_progress"></div>').appendTo(loader).append(progress);
            break;
        }
      }
    };
    ...
    this.cfg = function(config){
      cunij.extend(true, cfg, config);
    };

  };

  cunij.fn.unoslider = function(options){
    return (new cunij.UnoSlider(options, cunij(this)));
  };

  cunij.UnoSlider.defaults = {};
