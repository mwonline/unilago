...
defined('_JEXEC') or die;
	...
	class UniteImageViewHCar{
		...
		private function getThumbFilename(){
			...
			$dirname = UniteFunctionsHCar::getVal($info, "dirname");
			if(!empty($dirname)){
		...
		public function showImageFromGet(){
			...
			$imageFilename = UniteFunctionsHCar::getGetVar("img");
			
			$noencode = UniteFunctionsHCar::getGetVar("noencode","");
						
			if($noencode != "true")					
				$imageFilename = base64_decode($imageFilename);
			
			$maxWidth = UniteFunctionsHCar::getGetVar("w",-1);
			$maxHeight = UniteFunctionsHCar::getGetVar("h",-1);
			$type = UniteFunctionsHCar::getGetVar("t","");
			
			//set effect
			$effect = UniteFunctionsHCar::getGetVar("e");
			$effectArgument1 = UniteFunctionsHCar::getGetVar("ea1");
